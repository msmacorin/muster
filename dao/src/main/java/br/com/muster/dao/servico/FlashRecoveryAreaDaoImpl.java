 ...
 */
@Stateless
public class FlashRecoveryAreaDaoImpl extends Dao<FlashRecoveryArea, UUID> implements FlashRecoveryAreaDao {

    private static final long serialVersionUID = -8547467067885093756L;

    private final String SELECT_SITUACAO_ENVIO = "SELECT r FROM FlashRecoveryArea r "
            + " WHERE r.envio = :envio";

    private final String SELECT_SITUACAO_ENVIO_DATA = "SELECT r FROM FlashRecoveryArea r "
            + " WHERE r.envio = :envio "
            + " AND r.data < :data ";

    private final String SELECT_PERIOD = "SELECT r "
            + "FROM FlashRecoveryArea r "
    ...
    @Override
    public List<FlashRecoveryArea> registrosEnviadosAntigos() {
        try {
