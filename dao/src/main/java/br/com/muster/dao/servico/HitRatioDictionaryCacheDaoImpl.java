 ...
 */
@Stateless
public class HitRatioDictionaryCacheDaoImpl extends Dao<HitRatioDictionaryCache, UUID> implements HitRatioDictionaryCacheDao {

    private static final long serialVersionUID = 2000333820480550381L;

    private final String SELECT_SITUACAO_ENVIO = "SELECT r FROM HitRatioDictionaryCache r "
            + " WHERE r.envio = :envio";

    private final String SELECT_SITUACAO_ENVIO_DATA = "SELECT r FROM HitRatioDictionaryCache r "
            + " WHERE r.envio = :envio "
            + " AND r.data < :data ";

    private final String SELECT_PERIOD = "SELECT r "
            + "FROM HitRatioDictionaryCache r "
    ...
    @Override
    public List<HitRatioDictionaryCache> registrosEnviadosAntigos() {
        try {
