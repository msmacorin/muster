 ...
 */
@Stateless
public class ProcessoDaoImpl extends Dao<Processo, UUID> implements ProcessoDao{
    private static final long serialVersionUID = -8459680852392058361L;


    private final String SELECT_SITUACAO_ENVIO = "SELECT r FROM Processo r "
            + " WHERE r.envio = :envio";
    
    private final String SELECT_SITUACAO_ENVIO_DATA = "SELECT r FROM Processo r "
            + " WHERE r.envio = :envio "
            + " AND r.data < :data ";
    ...
    @Override
    public List<Processo> registrosEnviadosAntigos() {
        try {
